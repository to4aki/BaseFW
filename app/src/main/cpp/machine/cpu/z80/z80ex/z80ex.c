...
#include "ptables.c"
#include "machine/cpu/z80/z80ex/opcodes/opcodes_base.c"
#include "machine/cpu/z80/z80ex/opcodes/opcodes_dd.c"
#include "machine/cpu/z80/z80ex/opcodes/opcodes_fd.c"
#include "machine/cpu/z80/z80ex/opcodes/opcodes_cb.c"
#include "machine/cpu/z80/z80ex/opcodes/opcodes_ed.c"
#include "machine/cpu/z80/z80ex/opcodes/opcodes_ddcb.c"
#include "machine/cpu/z80/z80ex/opcodes/opcodes_fdcb.c"
...
static Z80EX_VERSION version = {Z80EX_API_REVISION, Z80EX_VERSION_MAJOR, Z80EX_VERSION_MINOR,
                                revision_type, ver_str};
...
LIB_EXPORT void
z80ex_set_tstate_callback(Z80EX_CONTEXT *cpu, z80ex_tstate_cb cb_fn, void *user_data) {
    cpu->tstate_cb = cb_fn;
...
LIB_EXPORT void
z80ex_set_memread_callback(Z80EX_CONTEXT *cpu, z80ex_mread_cb mrcb_fn, void *mrcb_data) {
    cpu->mread_cb = mrcb_fn;
    cpu->mread_cb_user_data = mrcb_data;
}

LIB_EXPORT void
z80ex_set_memwrite_callback(Z80EX_CONTEXT *cpu, z80ex_mwrite_cb mwcb_fn, void *mwcb_data) {
    cpu->mwrite_cb = mwcb_fn;
    cpu->mwrite_cb_user_data = mwcb_data;
}

LIB_EXPORT void
z80ex_set_portread_callback(Z80EX_CONTEXT *cpu, z80ex_pread_cb prcb_fn, void *prcb_data) {
    cpu->pread_cb = prcb_fn;
    cpu->pread_cb_user_data = prcb_data;
}

LIB_EXPORT void
z80ex_set_portwrite_callback(Z80EX_CONTEXT *cpu, z80ex_pwrite_cb pwcb_fn, void *pwcb_data) {
    cpu->pwrite_cb = pwcb_fn;
    cpu->pwrite_cb_user_data = pwcb_data;
}

LIB_EXPORT void
z80ex_set_intread_callback(Z80EX_CONTEXT *cpu, z80ex_intread_cb ircb_fn, void *ircb_data) {
    cpu->intread_cb = ircb_fn;
    cpu->intread_cb_user_data = ircb_data;
}
...
LIB_EXPORT int z80ex_int(Z80EX_CONTEXT *cpu) {
    ...
    switch (IM) {
        ...
        case IM2:
            ...
            READ_MEM(PCL, inttemp++, 13);
            READ_MEM(PCH, inttemp, 16);
            MEMPTR = PC;
            T_WAIT_UNTIL(19);
